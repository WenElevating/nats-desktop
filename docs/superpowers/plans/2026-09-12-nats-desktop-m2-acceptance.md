...
- 日期：2026-09-12（凌晨实跑）
- 分支 / HEAD：`desktop/m2` @ `adb41c8`（遗留清单经终审补全于后续提交）（Task 1–12 全部合并）+ 本记录与 flood 工具提交（Task 13）；终审修复（push-mode 徽标、发布历史载荷/时间、types.go 注释订正、规格偏差记录，见 §6「规格偏差显式记录（终审裁定）」）落地于 `adb41c8` 之后
...
| 规格条目 | 裁定 |
|---|---|
| §6.4 可选 header 过滤 | v1 未实现（客户端过滤在洪峰下需性能评估），显式延期至 M3，M3 计划须含实现与洪峰下过滤性能验证。 |
| §6.4 异常表「重连中会话标记暂停接收」 | 实现采用 M2 用户裁定（重连期间会话保持 running 显示、不标记暂停；重连成功自动恢复订阅），与规格文本的偏差经 2026-09-12 终审显式接受；M3 可评估增加"接收停滞"指示。 |

## 7. 控制器冒烟（2026-09-12 实测，UIA 驱动真实应用 + `nats://127.0.0.1:4333` 真服务器）

**发现并修复 Critical 缺陷 B2**：会话收流不触发 `session:state`（`deliver()` 缺 throttle.notify）——chip 永远停在创建时快照（0 msg/s 共 0 条），AC-005 速率显示与 AC-007 速率回落死路。修复 `5788412`（deliver 通知 + 静默期速率回落刷新），回归测试含真服务器变体，修复后重建应用复测通过。

| 冒烟项 | 结果 | 备注 |
|---|---|---|
| 启动自动恢复连接（M1 B1 回归） | **PASS** | 重启即 ESTABLISHED 到 4333 |
| AC-004 req echo 响应面板 + 历史 | **PASS** | UIA 填表→请求→`{"pong":true,…}`、耗时 **6 毫秒**、历史+清空历史按钮在位 |
| AC-005 实时上屏/速率计/累计 | **PASS** | 建会话→flood：流动中 chip 实测 **143 msg/s**（目标 150）、列表虚拟化渲染 #1..#N；修复前此处为死路（B2） |
| AC-006 暂停/恢复语义 | **PASS** | 暂停→显示冻结 4393（暂停期注入 600 条不上涨、按钮变恢复）；恢复→解冻至真实总数 4990、列表无回补 |
| AC-007 50k 洪峰 UI 响应性/丢弃角标 | **PASS（进程级+数据面）** | flood 实测 49,898 msg/s；洪峰中 chip 捕获 **41,753 msg/s / 共 46,743 / 已丢弃 36,743**（守恒精确 46,743=10,000+36,743、缓冲区 10,000 封顶）；洪峰后 UIA 立即恢复应答、进程存活。视觉流畅度（丢帧% 判读）留人工 |
| AC-029 二进制 hex 预览 + 下载 | **PENDING-MANUAL** | Go 侧（is_utf8/二进制路径）自动化已过；UI hex 视图人工走查（§5 附路径）。注：2MB 载荷受服务器默认 max_payload=1MB 限制被拒（应用呈现服务器错误原文——§6.3 预期行为），验证用 900KB |
| 清空按钮（UI） | **PENDING-MANUAL** | 50k 洪峰后 WebView2 UIA 提供方停止应答（应用进程/窗口正常，内存 156MB 合理），UI 交互冒烟中止；ClearSession 语义由 Go 测试覆盖 |
| 主题/语言持久化（M1 回归） | **PASS** | 重启后深色+zh-CN 保持 |

冒烟方法注记：本会话 computer-use 代理已停用，改用 Windows UIA（PowerShell UIAutomation：InvokePattern/ValuePattern/SelectionItemPattern/TogglePattern）驱动真实 WebView2 界面；物理点击注入因宿主上下文无交互桌面权限不可用。50k 洪峰后 UIA 提供方停止应答的现象已记录（应用本身存活），M3 可考虑为高频更新场景收敛 UIA 树暴露面积。
