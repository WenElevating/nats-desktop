...
|---|---|---|---|
| AC-001 | 全新目录启动进引导，八页可导航 | **AUTOMATED-PASS** + PENDING-MANUAL（视觉） | 测试：`shell.test.tsx` "renders all eight nav entries with svg icons"、`connstate-ui.test.tsx` "shows the guide card when no contexts exist; the sidebar stays navigable"。进程级：冷启动 5 轮均在无 context 状态下正常拉起。手动：① 关闭应用，将 `%USERPROFILE%\.config\nats` 与 `%APPDATA%\nats-desktop` 临时重命名；② 双击 `desktop\bin\nats-desktop.exe`；③ 期望引导卡片+侧栏 8 项可点+非设置页未连接空态，检查后还原目录。EN: rename both dirs, launch exe, expect guide card / 8-item sidebar / not-connected empty states. |
| AC-002 | 建 context → 测试连接（RTT+落盘） | **AUTOMATED-PASS** + PENDING-MANUAL（GUI） | 测试：Go `TestCheckConnection`（内嵌服务器断言 RTT 与 JetStream）、`TestCheckConnectionWithoutJetStream`、`TestCheckConnectionFailure`、`TestSaveThenList`（真实落盘 natscontext JSON）；前端 "shows the test result with rtt and jetstream"。手动：① 起本地服务器（`docker run -p 4222:4222 nats:latest` 或 desktop 目录 `go run github.com/nats-io/nats-server/v2`）；② Settings→Connections→新建 demo/`nats://127.0.0.1:4222`→测试连接；③ 期望 RTT 数值、列表出现 demo、`~/.config/nats/context/demo.json` 生成。 |
...
| AC-020 | 主题三态+系统跟随+重启保持 | **AUTOMATED-PASS** + PENDING-MANUAL（运行时） | 测试：`theme.test.ts`（light/dark/system/reject 4 例）、settings-page "saves picked theme and language"、`TestSaveLoadRoundTrip`。手动：Settings→Appearance 依次浅色/深色/跟随系统，确认即时生效（表格/对话框同步）；跟随系统下切换系统深浅模式 → 1s 内跟随；重启保持（此项同时覆盖 Task 2 遗留的 ThemeChanged 运行时冒烟）。 |
| AC-021 | zh-CN 全界面+重启保持 | **AUTOMATED-PASS** + PENDING-MANUAL（走查） | 测试：`i18n.test.ts`（zh-CN 键与 en 完全一致、无空值）+ CI i18n 门禁（desktop-ci.yml）。手动：切 zh-CN 浏览八页确认无英文残留；重启保持。 |
...
| §6.1 | 启动恢复（Task 10 台账：main.go wiring 无自动化测试） | **PENDING-MANUAL**（重点走查） | 半自动证据：`TestReconnectOnServerRestart`（连接自愈）、`TestAuthFailureGoesFailedNoRetryLoop`（失败不重试）覆盖 Manager 行为；main.go 的恢复 wiring（读取 LastActiveContext→goroutine 单次 Connect）无自动化。手动：① 连接任一 context 后重启应用（服务器在线）→ 自动恢复 connected、状态条 RTT 显示；② 停服务器后重启 → failed 一次、无重试循环、应用可正常导航，日志有 "startup connection restore failed" WARN。 |
