...
## §11 金标准复跑判定（2026-09-16 00:02 → 09-17 00:02，修复后构建 main@c407451）

**跑法**：soak-20260916-000205，1k msg/s × 1024B → soak.load，60s 导航循环，24h 满窗。verdict.txt 未生成（19:42 起驱动卡死在 UIA 调用，见下）——本节为手工判定，数据源 samples.csv / soak.log / flood.out.log / postmortem.txt / uia-probe。

**三件套**：
1. **崩溃 = 否（PASS）**：24h 存活，全程零 WER 事件（Application 1000/1001/1002 复查为空），无 crash.txt。
2. **内存增长 = FAIL**：1h 点 692MB → 终点 3,161MB（**+357%，≫10% 闸门**）。02:00→23:00 段 814→3,074MB，斜率 **~108MB/h 严格线性、无平台期**——证实 §10.1 的 40min 窗"尾段 166MB/h"是真实棘轮而非锯齿。
3. **句柄/线程 = PASS**：第 1 小时预热后全程走平（句柄 3,670-3,794，线程 160-170）。

**逐进程归属（新增实锤）**：终态 private——Go 主进程 227MB（24h 平稳，**修复 A 成立**：上轮同点位 14.3GB）；渲染子进程全部 ≤81MB 稳定；**增量全部在 WebView2 browser 进程（48140，终值 2,777MB）**。残余泄漏 B 的归属从"渲染器"修正为"browser 进程"。flood 全程证明：85,474,455 msgs / 98.9% 达成率 / 零错误。

**五点表**（树 private MB）：01:00=692 / 06:00=1,253 / 12:00=1,982 / 18:00=2,541 / 23:00=3,074（flood 停后终值 3,161）。

**新缺陷：UIA 渐进式冻结**——nav MISS 自 cycle ~854（~16:35）起连续全缺（终值 274/1008），19:42（cycle 1008 后）驱动卡死在 UIA FindFirst（soak.ps1 无超时包装）；24h 终态探针：顶层窗口 "NATS Desktop" 在、web 内容子树 **0 按钮**（正常 ≥8）→ 渲染侧内容树不可访问；后半段树 CPU ~44% 单核持续（flood 停后回落 ~17%）→ 渐进式渲染侧冻结，非 UI 线程即死。soak.ps1 改进项：FindFirst/FindAll 需超时包装（COM 调用可无限期阻塞），否则驱动永不收尾。

**日志冻结复现 + 归因修正**：应用日志 15:43:01 冻结（开跑 15.7h；上轮 57min）——起点可变，坐实非固定时长触发。末三组记录为 "sys watch created/stopped"（watch_id 91/92/93，~8min 周期即 Dashboard 轮询）→ **新主假设：日志静默可能不是 logger 自身死锁，而是前端停止发起监控轮询**（sys-watch INFO 由前端周期驱动）——即日志冻结或为渐进式渲染侧失效（与 UIA 冻结同源）的首个可见症状，时间线吻合（15:43 日志静 → 16:35 UIA 全 MISS → 19:42 UIA 全卡）。logger 独立死锁假设降级为陪跑候选，待专项探针证伪/证实。

**会话管线证据**：verdict §5 的 t0/t1 计数对未采集（evidence 点在卡死的 nav 循环内）→ 应用侧消费计数尾段不可证（诚实记录）；nats-server 侧发布全程成立。

**结论与后续**：AC-025 维持 FAIL（残余 ~108MB/h）。按用户标准（解决所有问题才能发布）发布继续阻塞。攻击顺序：① 残余泄漏 B 归因（browser 进程 24h full dump 已取证：`webview2-browser-48140-24h.dmp`；线索：500KB 级事件全走 >8KB parked-HTTP 通道经 browser 进程网络栈，但上轮 batch 模式同通道却平台期 → 差异点待查）；② 日志冻结（按新同源假设设计探针）；③ UIA 冻结归因（与①可能同根）。产物齐存于 soak-20260916-000205/。
